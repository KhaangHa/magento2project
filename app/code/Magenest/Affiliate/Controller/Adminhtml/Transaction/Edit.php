...
//         1. Get ID and create model
        ...
        $model = $this->_objectManager->create('Magenest\Affiliate\Model\Transaction');

        // 2. Initial checking
        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                $this->messageManager->addError(__('This Transaction no longer exits. '));
        ...
        $resultPage->getConfig()->getTitle()->prepend(__('Affiliate Transaction'));
